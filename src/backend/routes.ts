...
import { z } from "zod";
import { OfflineMLService, HybridLLMService } from "./ml";
...
import { initializeCiscoAPI, getCiscoAPI } from "./cisco-api-client";
import { APIEnrichmentService } from "./api-enrichment";
import { testTextGeneration, analyzeVulnerabilityData, generateSecurityRecommendations } from "./cisco-circuit";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api", (req, res) => {
    res.json({
      name: "Cisco Service Readiness Engineer AgenticOps Intelligence Dashboard API",
      version: "1.1.0",
  ...
  app.get("/api/reports/top-field-notices", async (req, res) => {
    try {
      ...
      if (month) {
        const allNotices = await storage.getTopFieldNoticesByMonth(month, 1000);
        totalCount = allNotices.length;
        notices = allNotices.slice(offset, offset + limit);
        period = month;
      } else {
        const allNotices = await storage.getTopFieldNoticesByYear(year, 1000);
        totalCount = allNotices.length;
        notices = allNotices.slice(offset, offset + limit);
        period = year?.toString() || 'all';
      }
  ...
  app.get("/api/reports/top-customers", async (req, res) => {
    try {
      ...
      if (month) {
        const allCustomers = await storage.getTopCustomersByMonth(month, 1000);
        totalCount = allCustomers.length;
        customers = allCustomers.slice(offset, offset + limit);
        period = month;
      } else {
        const allCustomers = await storage.getTopCustomersByYear(year, 1000);
        totalCount = allCustomers.length;
        customers = allCustomers.slice(offset, offset + limit);
        period = year?.toString() || 'all';
      }
  ...
  app.post("/api/export/pdf", async (req, res) => {
    try {
      ...
      doc.text("Service Readiness Engineer AgenticOps Intelligence Dashboard", { align: "center" });
      doc.moveDown(0.5);
      ...
      doc.text("Cisco Systems, Inc. | Service Readiness Engineer AgenticOps Intelligence Dashboard", 40, doc.page.height - 40, { align: "center", width: doc.page.width - 80 });
  ...
  app.post("/api/export/report/optimized", async (req, res) => {
    try {
      ...
      const optimizer = new PDFReportOptimizer(filename || "Service_Readiness_Engineer_AgenticOps_Intelligence_Report.pdf");
      
      // Set response headers
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", `attachment; filename="${filename || 'Service_Readiness_Engineer_AgenticOps_Intelligence_Report.pdf'}"`);
      
      // Pipe to response
      optimizer.getDocument().pipe(res);
      
      // Add header with title and timestamp
      optimizer.addHeader("Service Readiness Engineer AgenticOps Intelligence Report");
  ...
  app.get("/api/reports/ml-intelligence-summary", async (req, res) => {
    try {
      ...
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", "attachment; filename=\"Service_Readiness_Engineer_ML_Intelligence_Summary_3Page.pdf\"");
      res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
