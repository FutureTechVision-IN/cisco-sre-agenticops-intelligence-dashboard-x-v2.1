...
async function testAuth() {
  try {
    ...
    for (const account of accounts) {
      const res = await pool.query('SELECT username, role FROM users WHERE username = $1', [account]);
      if (res.rows.length > 0) {
        const user = res.rows[0];
        const matches = await bcrypt.compare(testPassword, (await pool.query('SELECT password FROM users WHERE username = $1', [account])).rows[0].password);
