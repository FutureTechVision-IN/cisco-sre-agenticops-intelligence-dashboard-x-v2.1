...
// Default CSV filename (used when pipeline hasn't selected an active dataset yet)
const DEFAULT_CSV_FILENAME = 'fn_aug25-feb26.csv';

// Active CSV path — set by the dataset pipeline when it elects a winner
let activeCSVPath: string | null = null;

/**
 * Set the active CSV file path (called by the dataset pipeline).
 * This overrides the default filename and ensures dashboard refresh
 * always uses the pipeline-elected active dataset.
 */
export function setActiveCSVPath(csvPath: string): void {
  activeCSVPath = csvPath;
  console.log('[CSV-CACHE] Active CSV path set to: ' + path.basename(csvPath));
}

// Data integrity validation constants
const EXPECTED_COLUMNS = ['FIELD_NOTICE', 'FIRST_PUBLISHED', 'FN_TITLE', 'FN_TYPE', 'TOT_VULN', 'CVUL', 'POT_VULN', 'CPVUL', 'NOT_VULN', 'CNVUL', 'DATE_IMPORTED', 'CPYKEY', 'CUSTOMER_NAME'];
const MIN_EXPECTED_RECORDS = 100000; // Minimum expected records for data integrity

/**
 * Get the CSV file path — returns the pipeline-elected active dataset,
 * or falls back to the default filename.
 */
export function getCSVPath(): string {
  if (activeCSVPath) return activeCSVPath;
  return path.join(process.cwd(), 'data', DEFAULT_CSV_FILENAME);
}
...
/**
 * Data Validation Pipeline - Validates CSV data integrity before loading
 * Checks: column structure, data completeness, consistency, and date range coverage
 */
export interface DataValidationResult {
  ...
  columnValidation: { expected: string[]; actual: string[]; missing: string[]; extra: string[] };
  dateRangeCoverage: { months: string[]; expectedMonths: string[]; missingMonths: string[]; coveragePercent: number };
  dataIntegrityChecks: { duplicateCount: number; nullFieldNotices: number; negativeValues: number; orphanedRecords: number };
  performanceMetrics: { parseTimeMs: number; validationTimeMs: number; totalTimeMs: number };
  timestamp: string;
}

async function validateCSVData(rawRecords: CSVRecord[], columns: string[]): Promise<DataValidationResult> {
  ...
  const extra = columns.filter(c => !EXPECTED_COLUMNS.includes(c));
  if (missing.length > 0) errors.push(`Missing columns: ${missing.join(', ')}`);
  if (extra.length > 0) warnings.push(`Extra columns: ${extra.join(', ')}`);
  
  // Data completeness checks
  let nullFieldNotices = 0;
  ...
  for (const record of rawRecords) {
    const fn = record.FIELD_NOTICE?.trim();
    const dateImported = record.DATE_IMPORTED?.trim() || '';
    const month = dateImported.substring(0, 7);
    
    if (!fn || fn === '') { nullFieldNotices++; invalidCount++; continue; }
    ...
    if (totVuln < 0 || potVuln < 0 || notVuln < 0) negativeValues++;
    
    // Check for orphaned records (no customer info)
    if (!record.CPYKEY?.trim() && !record.CUSTOMER_NAME?.trim()) orphanedRecords++;
    
    // Track months for coverage
    if (month && month.length === 7) monthSet.add(month);
    
    // Track duplicates
    const key = `${fn}|${record.CPYKEY}|${record.CUSTOMER_NAME}|${month}`;
    if (dedupKeys.has(key)) duplicateCount++;
    else dedupKeys.add(key);
    
    validCount++;
  }
  
  // Date range coverage validation — derive expected range from the data itself
  ...
  if (sortedMonths.length >= 2) {
    ...
    expectedMonths.push(sortedMonths[0]);
  }
  const missingMonths = expectedMonths.filter(m => !monthSet.has(m));
  const coveragePercent = ((expectedMonths.length - missingMonths.length) / expectedMonths.length) * 100;
  
  if (missingMonths.length > 0) warnings.push(`Missing month coverage: ${missingMonths.join(', ')}`);
  if (rawRecords.length < MIN_EXPECTED_RECORDS) warnings.push(`Record count ${rawRecords.length} below minimum expected ${MIN_EXPECTED_RECORDS}`);
  if (negativeValues > 0) errors.push(`Found ${negativeValues} records with negative vulnerability values`);
  if (nullFieldNotices > rawRecords.length * 0.1) errors.push(`${nullFieldNotices} records (${((nullFieldNotices/rawRecords.length)*100).toFixed(1)}%) have null field notices`);
  ...
  const result: DataValidationResult = {
    ...
    columnValidation: { expected: EXPECTED_COLUMNS, actual: columns, missing, extra },
    dateRangeCoverage: { months: Array.from(monthSet).sort(), expectedMonths, missingMonths, coveragePercent },
    dataIntegrityChecks: { duplicateCount, nullFieldNotices, negativeValues, orphanedRecords },
    performanceMetrics: { parseTimeMs: 0, validationTimeMs: Date.now() - startTime, totalTimeMs: 0 },
...
// Store last validation result for API access
let lastValidationResult: DataValidationResult | null = null;

export function getLastValidationResult(): DataValidationResult | null {
...
function normalizeRecord(raw: CSVRecord): NormalizedRecord {
  ...
  // Accept both BCS Field Notice IDs (FN#####) and PSIRT advisory IDs (cisco-sa-*, cisco-YYYYMMDD-*)
  const isPsirtAdvisoryId = /^cisco-/i.test(fieldNotice);
  const fieldNoticeFormatted = isPsirtAdvisoryId ? fieldNotice : formatFieldNoticeId(fieldNotice);
  const isValid = isPsirtAdvisoryId
    ? fieldNotice.length > 0
    : (isValidFieldNotice(fieldNotice) && fieldNoticeFormatted !== null);
...
export async function loadCSVData(forceReload: boolean = false): Promise<CacheState> {
  ...
  loadPromise = (async () => {
    ...
    console.log('[CSV-CACHE] Dataset: ' + path.basename(getCSVPath()));
    ...
    try {
      ...
      // Read and parse CSV — streaming to handle large files (1GB+) without OOM.
      // The `columns` callback remaps both PSIRT advisory and BCS lowercase column names
      // to canonical names in-stream, before any records enter memory.
      //
      // Month window precedence (only applied when an env var is explicitly set):
      //   1. CSV_START_MONTH env var — absolute lower bound (e.g. '2025-08')
      //   2. CSV_RECENT_MONTHS env var — rolling window of N months from today
      //   3. Default: load ALL records (no filter) — avoids silently dropping data
      //      when today's date has moved past the CSV's data range.
      let cutoffMonth: string | null = null;
      if (process.env.CSV_START_MONTH) {
        cutoffMonth = process.env.CSV_START_MONTH.trim();
        console.log(`[CSV-CACHE] Month window: loading records from ${cutoffMonth} onwards (CSV_START_MONTH)`);
      } else if (process.env.CSV_RECENT_MONTHS) {
        const recentMonths = parseInt(process.env.CSV_RECENT_MONTHS);
        const now = new Date();
        const cutoffDate = new Date(now.getFullYear(), now.getMonth() - (recentMonths - 1), 1);
        cutoffMonth = `${cutoffDate.getFullYear()}-${String(cutoffDate.getMonth() + 1).padStart(2, '0')}`;
        console.log(`[CSV-CACHE] Month window: loading records from ${cutoffMonth} onwards (last ${recentMonths} months)`);
      } else {
        console.log('[CSV-CACHE] Month window: loading ALL records (set CSV_START_MONTH or CSV_RECENT_MONTHS to limit)');
      }

      // Unified column alias map — covers PSIRT advisory schema and BCS lowercase variants.
      // Applied via the csv-parse `columns` callback so remapping happens in-stream
      // before any records are buffered, avoiding a second full-file pass.
      const COL_ALIAS_MAP: Record<string, string> = {
        // BCS lowercase / mixed-case variants
        'customer_name':              'CUSTOMER_NAME',
        'field_notice':               'FIELD_NOTICE',
        'fieldnotice':                'FIELD_NOTICE',
        'fn_type':                    'FN_TYPE',
        'fn_title':                   'FN_TITLE',
      ...
      const rawRecords: CSVRecord[] = await new Promise<CSVRecord[]>((resolve, reject) => {
        const records: CSVRecord[] = [];
        let psirtDetected = false;
        const parser = parseStream({
          // Remap both PSIRT and BCS lowercase column names → canonical names as header is parsed.
          columns: (header: string[]) => header.map(col => {
            const lower = col.toLowerCase().trim();
            if (COL_ALIAS_MAP[lower]) { psirtDetected = true; return COL_ALIAS_MAP[lower]; }
            return col.trim();
          }),
        ...
        parser.on('readable', function () {
          ...
          while ((rec = parser.read()) !== null) {
            // Month-window filter: only active when cutoffMonth is explicitly configured.
            if (cutoffMonth) {
              const dateVal = (rec.DATE_IMPORTED ?? '').trim();
              const month = dateVal.substring(0, 7);
              if (month && /^\d{4}-\d{2}$/.test(month) && month < cutoffMonth) continue;
            }
            records.push(rec);
          }
        });
        ...
        parser.on('end', () => {
          if (psirtDetected) console.log('[CSV-CACHE] PSIRT advisory schema detected — columns remapped in-stream');
          resolve(records);
        });
      ...
      console.log(`[CSV-CACHE] Parsed ${rawRecords.length} raw records in ${parseTime}ms`);

      // Run data validation pipeline
      const columns = rawRecords.length > 0 ? Object.keys(rawRecords[0]) : [];
      const validation = await validateCSVData(rawRecords, columns);
      validation.performanceMetrics.parseTimeMs = parseTime;
      lastValidationResult = validation;
      
      if (validation.validationErrors.length > 0) {
        console.warn('[CSV-CACHE] DATA VALIDATION ERRORS:', validation.validationErrors);
      }
      if (validation.validationWarnings.length > 0) {
        console.log('[CSV-CACHE] Data validation warnings:', validation.validationWarnings);
      }
      console.log(`[CSV-CACHE] Validation: ${validation.validRecords}/${validation.totalRecords} valid, ${validation.dateRangeCoverage.coveragePercent.toFixed(0)}% month coverage (${validation.dateRangeCoverage.months.length} months)`);
      ...
      const monthList = aggregations.availableMonths.join(', ');
      console.log(`[CSV-CACHE] Available months: ${monthList}`);
      console.log(`[CSV-CACHE] Data range: ${aggregations.availableMonths[0] || 'N/A'} to ${aggregations.availableMonths[aggregations.availableMonths.length - 1] || 'N/A'}`);
      ...
      validation.performanceMetrics.totalTimeMs = Date.now() - startTime;
      lastValidationResult = validation;
      
      console.log(`[CSV-CACHE] Total load time: ${Date.now() - startTime}ms`);
      console.log(`[CSV-CACHE] Cache stats: ${records.length} records, ${aggregations.uniqueCustomers.size} customers, ${aggregations.uniqueFieldNotices.size} field notices, ${aggregations.availableMonths.length} months`);
...
export async function getFilteredMetricsFromCache(
  ...
  // Also enrich the response with the actual dataset month range so callers
  // can display "Aug 2025 - Feb 2026" instead of a generic "All time" label.
  if (!filters.customer && !filters.fieldNotice && !filters.fnType && !filters.month && !filters.year) {
    const [metrics, cache] = await Promise.all([getMetricsFromCache(), loadCSVData()]);
    const months = cache.aggregations.availableMonths; // already sorted ascending
    ...
    const monthRange = firstMonth && latestMonth
      ? (firstMonth === latestMonth ? fmtMonth(firstMonth) : `${fmtMonth(firstMonth)} - ${fmtMonth(latestMonth)}`)
      : undefined;
    console.log(`[CSV-CACHE] getFilteredMetricsFromCache (no filters, ${monthCount} months) in ${Date.now() - startTime}ms`);
    return { ...metrics, firstMonth, latestMonth, monthCount, monthRange };
  }
