...
export function registerAdminRoutes(app: Express, storage: IStorage) {
  ...
  app.post("/api/admin/api-keys/:id/validate", async (req, res) => {
    try {
      ...
      const config = await storage.getApiKeyConfig?.(id);
      ...
      // Import the AI/ML Key Validator for real validation
      const { AIMLKeyValidator } = await import('./ai-ml-key-integration');
      
      // Perform real connectivity test
      const validation = await AIMLKeyValidator.testConnectivity(decryptedKey);

      if (validation.isValid) {
        res.json({
          status: validation.status,
          message: validation.message,
          connectivity: validation.details.connectivity,
          authentication: validation.details.authentication,
          permissions: validation.details.permissions,
          capabilities: validation.details.capabilities,
          rateLimit: validation.details.rateLimit,
        ...
        res.status(400).json({
          status: validation.status,
          message: validation.message,
          error: validation.details.connectivity === 'FAILED' 
            ? 'Connectivity issue - unable to reach API endpoint'
